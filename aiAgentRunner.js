...
let activeAgents = []; // 현재 접속 중인 에이전트 관리 배열
const MAX_CONCURRENT = 10; // 항상 상주할 인원 10명
...
class AIAgentClient {
    constructor(dbRow) {
        ...
        // 💡 개별 에이전트마다 1시간(60분) ~ 3시간(180분) 사이의 독립적인 랜덤 수명 부여
    ...
    startLoop() {
        ...
        this.loopTimer = setInterval(() => {
            // 💡 개별 에이전트의 수명이 다했을 때만 본인만 독립적으로 로그아웃 및 교체됨
...
// 💡 100명 풀 중 10명 상주 인원을 개별적으로 유지하고 관리하는 로테이션 매니저
async function manageAgentRotation() {
    try {
        // 연결이 끊겼거나 수명이 다해 소켓이 닫힌 에이전트들을 배열에서 안전하게 제거
        ...
        if (activeAgents.length < MAX_CONCURRENT) {
            ...
            for (let i = 0; i < needed && offlineList.length > 0; i++) {
                // 휴식 중인 에이전트 중 무작위로 한 명을 선택하여 투입
...
async function startRunner() {
    await initGroqModel();
    // 5초마다 인원을 체크하여 빈 자리가 생기면 새로운 에이전트를 독립적으로 투입
...
class AIAgentClient {
    ...
    setupListeners() {
       this.socket.on('server_shutdown_notice', async () => {
            ...
            try {
                await supabase.from('characters').update({ data: { player: this.charData } }).eq('id', this.dbRow.id);
            } catch (e) {}
            setTimeout(() => {
                process.exit(0);
            }, 1000);
